...
```
```{r}
water <- water %>% select(-c(is_urban,water_point_population,local_population_1km,distance_to_primary_road,distance_to_tertiary_road,staleness_score))
```
...
write_csv(water_ts_pred[,c("ID", "pstatus_id")], file="predictions18.csv")
```
