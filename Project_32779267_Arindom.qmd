...
```
```{r}
water_success_rate <- water %>% count(water_tech_category,status_id) %>%
  group_by(water_tech_category) %>% mutate(Success_rate = n[status_id=="y"]/sum(n)) |>
  summarise(Success_rate = mean(Success_rate)) |>
  select(water_tech_category,Success_rate) %>% kbl()
