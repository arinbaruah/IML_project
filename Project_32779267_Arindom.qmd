...
```{r}
water <- water %>% select(-is_urban,water_point_population,local_population_1km,distance_to_primary_road)
water_test <- water_test %>% select(-is_urban,water_point_population,local_population_1km,distance_to_primary_road)
```
...
write_csv(water_ts_pred[,c("ID", "pstatus_id")], file="predictions14.csv")
```
